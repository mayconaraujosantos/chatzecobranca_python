...
from src.main.domain.entities.conversation_state import (
    ConversationManager,
    ConversationState,
)
...
class ProcessWebhookMessage:
    def __init__(
        self,
        chatpro_client: ChatProHttpClient,
    ...
    def execute(self, message_data: dict) -> dict:
        try:
            ...
            from_number = (
                from_whatsapp.split("@")[0] if "@" in from_whatsapp else from_whatsapp
            )
...
Digite o número da opção desejada:
    ...
    def _handle_main_menu(
        self, phone_number: str, choice: str, message_id: str
    ) -> dict:
        choice = choice.strip()

        if choice == "1":
            self.conversation_manager.set_state(
                phone_number, ConversationState.CONSULT_DEBTS
            )
            ...
            self.conversation_manager.set_state(
                phone_number, ConversationState.PAY_BILL
            )
            response = self._send_message(
                phone_number,
            ...
            response = self._send_message(
                phone_number, "👋 Obrigado por usar o ZéCobrança! Até logo!", message_id
            )
            return response
    ...
    def _handle_consult_debts(
        self, phone_number: str, document: str, message_id: str
    ) -> dict:
        ...
        if document.strip():
            ...
            response = self._send_message(
                phone_number, "❌ Por favor, digite um CPF ou CNPJ válido.", message_id
            )
            return response

    def _handle_pay_bill(
        self, phone_number: str, barcode: str, message_id: str
    ) -> dict:
        ...
        if barcode.strip():
            ...
            response = self._send_message(phone_number, response_msg, message_id)
            self.conversation_manager.set_state(
                phone_number, ConversationState.MAIN_MENU
            )
            return response
        else:
            response = self._send_message(
    ...
    def _send_message(
        self, phone_number: str, message: str, quoted_message_id: str
    ) -> dict:
        chat_message = ChatMessage(
            number=phone_number, message=message, quoted_message_id=quoted_message_id
        )
